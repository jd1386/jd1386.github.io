...
유저의 viewport에 특정 컴포넌트가 존재하는지 여부를 쉽게 확인하도록 도와주는 [react-on-screen](https://github.com/fkhadra/react-on-screen)을 활용해서 컴포넌트 lazy load를 구현해보자. 먼저 아래 데모 사이트를 확인해보고 우리가 만들려고 하는 게 뭔지 살펴보자.
...
여기서 핵심은 각 컴포넌트가 유저의 viewport에 있는 지 없는 지를 `react-on-screen`을 활용해서, viewport 안에 있다면 API call해서 해당 리스트를 렌더하고 viewport에 밖에 있다면 아무 것도 하지 않는다는 것이다. 
